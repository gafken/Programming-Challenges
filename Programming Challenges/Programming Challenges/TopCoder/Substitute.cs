...
namespace Programmr_Challenges.TopCoder
{
    public static class Substitute
    {
        ...
        {
            Console.WriteLine(getValue("TRADINGFEW", "LGXWEV") == 709);
            Console.WriteLine(getValue("ABCDEFGHIJ", "XJ") == 0);
            Console.WriteLine(getValue("CRYSTALBUM", "MMA") == 6);
            Console.ReadLine();
        }

        public static int getValue(string key, string code)
        {
            var Key = key.ToCharArray();
            ...
            foreach (var x in blah)
                moreBlahs += x.ToString();

            if (string.IsNullOrWhiteSpace(moreBlahs))
                moreBlahs = "0";

            return Int32.Parse(moreBlahs);
        }
    }
}
