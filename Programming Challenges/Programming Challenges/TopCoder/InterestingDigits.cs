...
{
    public static class InterestingDigits
    {
        #region Problem
        ...
        {
            Console.WriteLine(digits(10).SequenceEqual(new[] { 3, 9 }));
            Console.WriteLine(digits(3).SequenceEqual(new[] { 2 }));
            Console.WriteLine(digits(9).SequenceEqual(new[] { 2, 4, 8 }));
            Console.WriteLine(digits(26).SequenceEqual(new[] { 5, 25 }));
            Console.WriteLine(digits(30).SequenceEqual(new[] { 29 }));
            Console.ReadLine();
        }
        #endregion

        public static int[] digits(int baseInt)
        {
            baseInt--;

            var result = new List<int>();

            for (int i = 2; i < baseInt + 1; i++)
            {
                if (baseInt % i == 0 && !result.Contains(i))
                    result.Add(i);
            }
